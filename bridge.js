...
function ocRequest(path, options = {}, timeoutMs = TIMEOUT_MS) {
  return new Promise((resolve, reject) => {
    const url = new URL(`${OPENCODE_URL}${path}`)
    ...
    const request = requestFn(url, requestOptions, (response) => {
      ...
      response.on("end", () => {
        resolve({ ok: (response.statusCode ?? 0) >= 200 && (response.statusCode ?? 0) < 300, status: response.statusCode ?? 0, text })
      })
    })
...
async function ocPost(path, body) {
  ...
  const res = await ocRequest(path, {
    method:  "POST",
